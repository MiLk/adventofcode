...
def fill_operators(numbers: list[int]) -> Iterable[list[int | str]]:
    if len(numbers) == 1:
        yield [numbers[0]]
        return
    heads, tail = numbers[:-1], numbers[-1]
    for filled in list(fill_operators(heads)):
        yield filled + ["+", tail]
        yield filled + ["*", tail]
        yield filled + ["||", tail]


def evaluate(expression: list[int | str]) -> int:
    if len(expression) == 1:
        return expression[0]
    first, operator, second, tail = expression[0], expression[1], expression[2], expression[3:]
    match operator:
        case "+":
            return evaluate([first + second] + tail)
        case "*":
            return evaluate([first * second] + tail)
        case "||":
            return evaluate([int(f"{first}{second}")] + tail)
        case _:
