...
def parse_line(line: str) -> tuple[int, GameDraws]:
    ...
    game_id = int(game[5:])
    draws = [
        dict(parse_cube(cube) for cube in draw.strip().split(", "))
...
def is_possible_game(draws: GameDraws, load: tuple[int, int, int]) -> bool:
    r, g, b = load
    return all(
        draw.get("r", 0) <= r and draw.get("g", 0) <= g and draw.get("b", 0) < b
        for draw in draws
    )


async def main():
    games = dict(parse_line(l) for l in read_input())
    print(
        sum(
            game_id
            for game_id, draws in games.items()
            if is_possible_game(draws, (12, 13, 14))
        )
    )

    print(
        sum(
            max(d.get("r", 0) for d in draws)
            * max(d.get("g", 0) for d in draws)
            * max(d.get("b", 0) for d in draws)
